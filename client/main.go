...
var totalPingsPerConnection uint64 = 10000
var concurrentConnections uint64 = 2
...
func (c *client) writeLoop(wg *sync.WaitGroup) {
	...
	wr := bufio.NewWriterSize(c.conn, 1024*1024)
	// outBuf := []byte("GET key\r\n")
	...
	for atomic.LoadUint64(&c.sent) < totalPingsPerConnection {
		// outBuf = []byte(fmt.Sprintf("SET %s 0 %s\r\n", hex.EncodeToString(keybuf), hex.EncodeToString(valbuf)))
		// outBuf = []byte(fmt.Sprintf("SET %d 0 %d\r\n", c.sent, c.revcd))
...
func NewClient(wg *sync.WaitGroup) {
	...
	if err != nil {
		fmt.Println("client creation: ", err)
		return
	}
